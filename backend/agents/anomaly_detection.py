...
Uses Z-score + IQR to flag statistical outliers, then LLM explains WHY.
"""
...
from tools.json_utils import extract_json_array
...
async def detect(ingestion_result: dict, run_id: Optional[str] = None) -> dict:
    ...
    raw = llm_result["text"]
    explanations = extract_json_array(raw) or []
    ...
    logger.info("[AnomalyDetection] Flagged %d anomalies  [provider=%s, %dms]",
                 len(enriched), llm_result.get("provider", "?"), llm_result.get("duration_ms", 0))
