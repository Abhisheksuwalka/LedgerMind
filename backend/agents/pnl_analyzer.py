...
LLM writes the human-readable narrative.
"""
...
# Configurable via .env — defaults to 25%
...
def _compute_pnl(transactions: list[dict]) -> dict:
    ...
    # Net profit after tax (rate set via TAX_RATE_PCT env var)
...
async def analyze(ingestion_result: dict, run_id: Optional[str] = None) -> dict:
    ...
    raw = llm_result["text"]
    parsed = extract_json_object(raw) or {"narrative": raw}

    logger.info(
        "[PnLAnalyzer] Done — revenue=$%.2f, net_profit=$%.2f  [provider=%s, %dms]",
        pnl_numbers["revenue"],
