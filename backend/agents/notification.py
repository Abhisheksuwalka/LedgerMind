...
FROM_EMAIL = os.getenv("SENDGRID_FROM_EMAIL", "reports@ledgermind.ai")
ALERT_RECIPIENT = os.getenv("NOTIFICATION_EMAIL", os.getenv("ALERT_EMAIL_RECIPIENT", "team@ledgermind.ai"))
...
def _build_report_email(state: dict) -> Mail:
    ...
    html_body = f"""
    <html><body>
    <h2>LedgerMind — Financial Report Ready</h2>
    ...
    <p><em>Full report available in the LedgerMind dashboard.</em></p>
    </body></html>
    """
    ...
    return Mail(
        ...
        subject=f"LedgerMind — Report Ready (Run {run_id[:8]})",
...
def _build_anomaly_alert_email(state: dict) -> Mail:
    ...
    html_body = f"""
    <html><body>
    <h2 style="color:red;">⚠️ LedgerMind — Critical Anomaly Alert</h2>
    ...
    <p><em>Review in the LedgerMind dashboard immediately.</em></p>
    </body></html>
    """
    ...
    return Mail(
        from_email=FROM_EMAIL,
        to_emails=ALERT_RECIPIENT,
        subject=f"🚨 CRITICAL: {critical} Anomalies Detected — LedgerMind",
        html_content=html_body,
    )
