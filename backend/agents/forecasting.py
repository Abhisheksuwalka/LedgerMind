...
Projects 30/60/90-day cash flow using linear trend + LLM interpretation.
"""
...
async def forecast(ingestion_result: dict, run_id: Optional[str] = None) -> dict:
    ...
    raw = llm_result["text"]
    parsed = extract_json_object(raw) or {"summary": raw}

    logger.info(
        "[ForecastingAgent] Projections: 30d=$%.2f, 60d=$%.2f, 90d=$%.2f  [provider=%s, %dms]",
