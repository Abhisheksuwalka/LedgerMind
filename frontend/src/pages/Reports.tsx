...
export default function Reports() {
  ...
  return (
    <div className="space-y-6">
      ...
      {reports.length === 0 && (
        <p className="text-slate-500">No reports yet. Upload a CSV and run the pipeline to generate one.</p>
      )}
      ...
      {loading && <p className="text-slate-500 animate-pulse">Loading report…</p>}
      ...
      {selected && !loading && (
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-6">
          ...
          {/* Rendered Markdown (E3) */}
          <div className="prose prose-invert prose-sm max-w-none max-h-[65vh] overflow-y-auto pr-2">
            <ReactMarkdown>
              {selected.markdown_report || selected.executive_summary || '_No report content available._'}
            </ReactMarkdown>
          </div>
        </div>
      )}
